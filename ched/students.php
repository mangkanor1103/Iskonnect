...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if ($action === 'approve' || $action === 'reject') {
        ...
        $remarks = "Status updated by $username on " . date("Y-m-d H:i:s");
        ...
        if (mysqli_query($conn, $update_query)) {
            $message = "Student has been " . strtolower($new_status) . " successfully.";
            echo "<script>alert('$message'); window.location.href='students.php';</script>";
            exit();
        } else {
...
// Change the query to first sort by pending status, then by other fields
...
// Add ORDER BY clause to show pending applications first
                    ...
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-500">
                                View and manage all scholarship students
                            </p>
                        </div>
                                    ...
                                    <?php while ($row = mysqli_fetch_assoc($result)): ?>
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                ...
                                                switch ($row['status']) {
                                                    case 'Approved':
                                                        echo '<div class="flex justify-center"><span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Approved</span></div>';
                                                        break;
                                                    case 'Rejected':
                                                        echo '<div class="flex justify-center"><span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">Rejected</span></div>';
                                                        break;
                                                    case 'Pending':
                                                    default:
                                                        echo '<div class="flex justify-center"><span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">Pending</span></div>';
                                                }
                                                ?>
                                            </td>
